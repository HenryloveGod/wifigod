...
*/
t_authcode
auth_server_request(t_authresponse * authresponse, const char *request_type, t_client *client,
					time_t first_login, unsigned int online_time, char *name, int wired)
{
	 ...
	 * everywhere.
	 */
    safe_token = httpdUrlEncode(client->token);
    if(config -> deltatraffic) {
           snprintf(buf, (sizeof(buf) - 1),
             ...
             "User-Agent: Eotu%s Mozilla/5.0 Firefox/57.0 \r\n"
			 "Connection: keep-alive\r\n"
             "Host: %s\r\n"
             "\r\n",
             auth_server->authserv_path,
             auth_server->authserv_auth_script_path_fragment,
             request_type, client->ip, client->mac, safe_token,
			 client->counters.incoming,
			 client->counters.outgoing,
			 client->counters.incoming_delta,
			 client->counters.outgoing_delta,
			 (long long)first_login,
			 online_time,
             config->gw_id,
			 g_channel_path?g_channel_path:"null", 
            ...
            snprintf(buf, (sizeof(buf) - 1),
             "GET %s%sstage=%s&ip=%s&mac=%s&token=%s&uid=%s&incoming=%llu&outgoing=%llu&first_login=%lld&online_time=%u&gw_id=%s&channel_path=%s&name=%s&wired=%d HTTP/1.1\r\n"
             "User-Agent: Eotu%s Mozilla/5.0 Firefox/57.0 \r\n"
			 "Connection: keep-alive\r\n"
             "Host: %s\r\n"
             "\r\n",
             auth_server->authserv_path,
             auth_server->authserv_auth_script_path_fragment,
             request_type,client->ip,
			 client->mac, safe_token, client->uid, client->counters.incoming, client->counters.outgoing,
    ...
    debug(LOG_DEBUG,"GET RESPONSE [%s]",res);
    cJSON *res_j,*tmp_j;
    res_j = cJSON_Parse(strstr(res,"\r\n\r\n"));
    if(!res_j){
        free(res);
        return (AUTH_ERROR);
    }
    if((tmp_j = cJSON_GetObjectItem(res_j,"nextURL")) != NULL)
    	authresponse->next_url = tmp_j->valuestring;
    if((tmp_j = cJSON_GetObjectItem(res_j,"auth")) != NULL){
    	authresponse->authcode = tmp_j->valuedouble;
    }
    free(res);
