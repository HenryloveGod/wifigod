...
typedef struct {
	...
	char *global_white;
	char *global_black;
	char *remind_sms_content;
	char *login_skip_host;
	char *serialnumber;
	char *mac;
	char *key;
	char *created;
	...
	int baudrate;
	int logsize;
	int auth_down_max_to_reboot;
	char *app_conf_path;

	char *sync_server; //同步服务器地址
	...
	char *sign;
	int ap_to_commit;
	int bootv; // 0  1   2 network restart  3  4
	long sign_time;
	int is_sms_work;
	char *sms_err_info;
	int sta_to_commit;
