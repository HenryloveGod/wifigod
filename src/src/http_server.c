...
http_403_callback(struct evhttp_request *req, void *arg) {
	...
	const char *path;
	char *decoded_path = malloc(1024);
	char *whole_path = NULL;
	size_t len;
	int fd = -1;
	struct stat st;

	debug(LOG_INFO,"403 handle");
...
//	}
	...
	safe_asprintf(&whole_path, "%s%s", docroot,decoded_path+1);

	debug(LOG_INFO,"whole_path =  %s",whole_path);
...
err:
	evhttp_send_error(req, 404, "BAD ! Document was not found");
	if (fd >= 0)
		close(fd);
done:
	if (decoded)
