...
const char	*g_inner_trusted_domains = "eotu.com,wx-s.net";
...
config_read(const char *filename)
{
	...
	if (!(fd = fopen(filename, "r"))) {
		debug(LOG_ERR, "Could not open configuration file '%s', " "not exiting...", filename);
		//exit(1);
		return ;
	}
...
validate_popular_servers(void)
{
	if (config.popular_servers == NULL) {
		...
		add_popular_server("www.eotu.org");
