...
#include "mcJSON.h"    //eotu alter json-c with this
#include "gsclient_handle.h"    //eotu alter json-c with this

#ifdef	_MQTT_SUPPORT_

//static void process_mqtt_reqeust(struct mosquitto *mosq,
//		const unsigned int req_id, const char *data, s_config *config);
static void mqtt_server_handle(struct mosquitto *mosq,char *data,s_config *config,int req_i);



static struct wifidog_mqtt_op {
	char *operation;
	void (*process_mqtt_op)(void *, const char *, const char *, const int,
			const s_config *);
} mqtt_op[] = {
...
} mqtt_set_type[] = { { "domain", add_trusted_domains }, { "pdomain",
		add_trusted_pdomains }, { "ip", add_trusted_iplist }, { "mac",
		add_trusted_maclist }, { NULL, NULL } };

static struct wifidog_mqtt_del_type {
	char *type;
	void (*process_mqtt_del_type)(const char *args);
} mqtt_del_type[] = { { "domain", del_trusted_domains }, { "pdomain",
		del_trusted_pdomains }, { "ip", del_trusted_iplist }, { "mac",
		del_trusted_maclist }, { NULL, NULL } };

static struct wifidog_mqtt_clear_type {
	char *type;
	void (*process_mqtt_clear_type)(void);
} mqtt_clear_type[] = { { "domain", clear_trusted_domains }, { "pdomain",
		clear_trusted_pdomains }, { "ip", clear_trusted_iplist }, { "mac",
		clear_trusted_maclist }, { NULL, NULL } };

static struct wifidog_mqtt_show_type {
	char *type;
	char *(*process_mqtt_show_type)(void);
} mqtt_show_type[] = { { "domain", show_trusted_domains }, { "pdomain",
		show_trusted_pdomains }, { "ip", show_trusted_iplist }, { "mac",
		show_trusted_maclist }, { NULL, NULL } };

static void send_mqtt_response(struct mosquitto *mosq,
		const unsigned int req_id, int res_id, const char *msg,
		const s_config *config) {
	char *topic = NULL;
	char *res_data = NULL;
	//safe_asprintf(&topic, "wifidog/%s/response/%d", config->gw_id, req_id);
	safe_asprintf(&topic, "wifidog/ping/%d/%d",config->id,req_id);
//	safe_asprintf(&res_data, "{\"response\":\"%d\",\"msg\":\"%s\"}", res_id,
//			msg == NULL ? "null" : msg);
	res_data=msg;
	debug(LOG_DEBUG, "send mqtt response: topic [%s] msg [%s]", topic,res_data);
...
void set_trusted_op(void *mosq, const char *type, const char *value,
	...
	for (int i = 0; mqtt_set_type[i].type != NULL; i++) {
		if (strcmp(mqtt_set_type[i].type, type) == 0) {
			debug(LOG_DEBUG, "set trusted operation, type is %s value is %s",
					type, value);
...
void del_trusted_op(void *mosq, const char *type, const char *value,
		const int req_id, const s_config *config) {
	if (!type || !value) {
	...
	for (int i = 0; mqtt_del_type[i].type != NULL; i++) {
		if (strcmp(mqtt_del_type[i].type, type) == 0) {
			debug(LOG_DEBUG, "del trusted operation, type is %s value is %s",
					type, value);
...
void clear_trusted_op(void *mosq, const char *type, const char *value,
		const int req_id, const s_config *config) {
	if (!type) {
...
void show_trusted_op(void *mosq, const char *type, const char *value,
		const int req_id, const s_config *config) {
	if (!type) {
...
void get_status_op(void *mosq, const char *type, const char *value,
		const int req_id, const s_config *config) {
...
void reboot_device_op(void *mosq, const char *type, const char *value,
		const int req_id, const s_config *config) {
	system("reboot");
}

void reset_device_op(void *mosq, const char *type, const char *value,
		const int req_id, const s_config *config) {
	t_client *p = NULL;

	debug(LOG_DEBUG, "value is %s and type is %s received from mqtt", value,
			type);
...
void set_auth_server_op(void *mosq, const char *type, const char *value,
	...
	json_object * jo_host_name = json_object_object_get(json_request,
			"hostname");
	...
	debug(LOG_DEBUG, "tmp_host_name is %s, tmp_http_port is %s, tmp_path is %s",
			tmp_host_name, tmp_http_port, tmp_path);
	...
	if ((tmp_host_name != NULL) && (strcmp(hostname, tmp_host_name) != 0)) {
		...
		uci_set_value("wifidog", "wifidog", "auth_server_hostname",
	...
	if ((tmp_path != NULL) && (strcmp(path, tmp_host_name) != 0)) {
		...
		uci_set_value("wifidog", "wifidog", "auth_server_path",
...
static void process_mqtt_reqeust(struct mosquitto *mosq,
	...
	const char *op = json_object_get_string(
			json_object_object_get(json_request, "op"));
	if (!op) {
	...
	for (; mqtt_op[i].operation != NULL; i++) {
		if (strcmp(op, mqtt_op[i].operation) == 0
				&& mqtt_op[i].process_mqtt_op) {
			const char *type = json_object_get_string(
					json_object_object_get(json_request, "type"));
			const char *value = json_object_get_string(
					json_object_object_get(json_request, "value"));
...
//wifidog/response/40/req?1

int get_topic_req_id(const char *topic) {
...
/***
 * 处理数据后，发送主题
 * wifidog/ser_pub_res/%d?%d",config->id,req_i
 * ***/
static void mqtt_server_handle(struct mosquitto *mosq,char *data,s_config *config,int req_i){
	cJSON *dj,*resj;
	char *res;
	...
	safe_asprintf(&restopic,"wifidog/cli_pub/%d/back?%d",config->id,req_i);

	if((dj=cJSON_Parse(data)) == NULL){
		debug(LOG_INFO,"%s parse error!",data);
		res = strdup("cJSON Parse error!");
	}else{
		resj = client_set_and_request(dj);
		/****返回处理结果*****/
		if((res=cJSON_Print(resj)) != NULL){

			cJSON_Delete(resj);
		}
		cJSON_Delete(dj);
	}
	debug(LOG_INFO,"mqtt_server_handle start to publish -----topic[%s] send [%s]",restopic,res);

	mosquitto_publish(mosq, NULL, restopic, strlen(res), res, 0, false);
//	free(restopic);
//	free(res);
}
...
static void mqtt_message_callback(struct mosquitto *mosq, void *obj,
	...
	if (message->payloadlen) {
		debug(LOG_DEBUG, "\r\n\r\n client recieved:-----------topic[%s], data [%s]\r\n\r\n", message->topic,
				message->payload);
		req_id = get_topic_req_id(message->topic);
		mqtt_server_handle(mosq,(char *)message->payload,config,req_id);
...
static void mqtt_connect_callback(struct mosquitto *mosq, void *obj, int rc) {
	...
	//safe_asprintf(&default_topic, "wifidog/%d/request/+", config->id);
	safe_asprintf(&default_topic, "wifidog/ser_pub/%d/#",config->id);
	mosquitto_subscribe(mosq, NULL, default_topic, 0); // qos is 0
	//一旦连接，即注册
	//send_mqtt_response(mosq, config->id, 200, "reg", config);
	free(default_topic);
}

int pw_callback(char *buf, int size, int rwflag, void *userdata){
	debug(LOG_INFO, "pw_callback =========%s\r\n %d\r\n  %d\r\n %s \r\n",buf,size,rwflag,(char *)userdata);
	return 0;
}
...
void thread_mqtt(void *arg) {
	...
	char *id;
	safe_asprintf(&id,"%d",config->id);
	mosq = mosquitto_new(id, true, config);
	if (mosq == NULL) {
		switch (errno) {
		case ENOMEM:
	...
	enum mosq_err_t  res=-10 ;

	mosquitto_connect_callback_set(mosq, mqtt_connect_callback);
	mosquitto_message_callback_set(mosq, mqtt_message_callback);

	switch (retval = mosquitto_connect(mosq, "127.0.0.1", 1883, keepalive)) {
	case MOSQ_ERR_INVAL:
	...
	debug(LOG_INFO, "INFO: mosquitto_loop_forever return :%d\n", retval);
	mosquitto_destroy(mosq);
	mosquitto_lib_cleanup();
}
