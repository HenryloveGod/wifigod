...
update_trusted_mac_list_status(void)
{
	...
	for(p1 = tmac_list; p1 != NULL; p1 = p1->next) {
		...
		if (config->auth_servers != NULL && p1->is_online) {
			t_client *tcl;
			tcl = malloc(sizeof(t_client));
			tcl->mac = p1->mac;
			tcl->ip = p1->ip;

			auth_server_request(&authresponse, REQUEST_TYPE_COUNTERS,tcl, 0, 0, "null", br_is_device_wired(p1->mac));
	...
	for (p1 = p2 = worklist; NULL != p1; p1 = p2) {
		...
		if (config->auth_servers != NULL && p1->is_online) {
			auth_server_request(&authresponse, REQUEST_TYPE_COUNTERS, p1,
								...
								p1->first_login,
								(p1->counters.last_updated - p1->first_login),
