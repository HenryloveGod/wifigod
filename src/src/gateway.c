...
struct evbuffer *evb_eotu_portal_page	= NULL;
...
init_web_server(s_config *config)
{
    ...
    * eotu own
    * **/
    httpdAddCContent(webserver, "/app", "avlist", 0, NULL, http_eotu_avlist);
    httpdAddCContent(webserver, "/app", "checkuid", 0, NULL, http_eotu_checkuid);
    httpdAddCContent(webserver, "/app", "auth", 0, NULL, http_eotu_auth);
    httpdAddCContent(webserver, "/", "wifiguid", 0, NULL, http_eotu_guid);
	//httpdAddCContent(webserver, "/app", "god", 0, NULL, http_godbless);
...
main_loop(void)
{
    ...
    while (1) {
    	...
    	debug(LOG_DEBUG, "Waiting connections ---------------");

        r = httpdGetConnection(webserver, NULL);

        debug(LOG_DEBUG, "get connections ---------------");
...
gw_main(int argc, char **argv)
{
    ...
    /*
     * 注册后 开始 主程序，没有注册完，一直循环;
     * json配置中ssid等参数改变，会影响启动。所以在此添加
     * */
    debug(LOG_NOTICE, "START TO REGIST , while loop until success! Frequence :1/min");
    register_loop();
    debug(LOG_NOTICE, "END REGISTER");
    boot_handle();
	config->bootv = 0;
