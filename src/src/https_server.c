 ...
 * ****/
char *
evhttpd_get_full_redir_url_eotu(request *r , const char *orig_url) {
    ...
    if(r->request.is_eotu ==1){
    	evbuffer_add_printf(evb, "%s://%s?router_id=%d&gw_address=%s&gw_port=%d&gw_id=%s&ip=%s&mac=%s&uid=%s&eotu=%d&url=%s",
    					protocol, config->portal_url,config->id,
    					config->gw_address, config->gw_port, config->gw_id,
    					r->clientAddr, r->mac, r->uid,r->request.is_eotu,orig_url);
    }else{
    	evbuffer_add_printf(evb, "%s://%s?router_id=%d&gw_address=%s&gw_port=%d&gw_id=%s&ip=%s&mac=%s&eotu=%d&url=%s",
    					protocol, config->portal_url,config->id,
    					config->gw_address, config->gw_port, config->gw_id,
    					r->clientAddr, r->mac, r->request.is_eotu,orig_url);
    }
...
evhttp_gw_reply_js_redirect(struct evhttp_request *req, const char *peer_addr) {
	...
	r->mac = mac;
	strncpy(&r->clientAddr,peer_addr,HTTP_IP_ADDR_LEN);

	char *redir_url = evhttpd_get_full_redir_url_eotu(r, req_url);
	struct evbuffer *evb = evbuffer_new();
...
void thread_https_server(void *args) {
	s_config *config = config_get_config();
	debug (LOG_ERR, "handle ~~~~~~~~thread_https_server \n");
