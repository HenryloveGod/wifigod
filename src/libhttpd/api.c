...
/*eotu app interface FOR ios/Android*/

void eotu_av_response(request * r,char *buff){

	_httpd_sendHeaders(r, strlen(buff), 0);
	_httpd_sendText(r, buff);
	close(r->clientSock);
    ...
    while (_httpd_readLine(r, buf, HTTP_MAX_LEN) > 0) {
        ...
        if (inHeaders) {
			...
			if (strncasecmp(buf, "User-Agent:", 11) == 0) {
                ...
                if (cp) {
                    cp += 2;
                    strncpy(r->request.user_agent, cp, HTTP_MAX_USER_AGENT);
                    r->request.user_agent[HTTP_MAX_USER_AGENT - 1] = 0;
                    r->request.is_eotu=0;
                    if(strstr(r->request.user_agent,"Eotu")){
                    	r->request.is_eotu=1;
                    }
                }
			}
